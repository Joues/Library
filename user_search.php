...
<form name="form3" method="post" action="">
      <table width="800" height="400" align="center" bgcolor="#99CC99"><tr>
        <td height="100" align="center" valign="middle">
          <p>按图书的类别查询：<input type="text" name="book_type" placeholder="类别">&nbsp;<input type="submit" name="submit1" value="开始查询"></p>
          <p>按图书出版社查询：<input type="text" name="book_come" placeholder="出版社">&nbsp;<input type="submit" name="submit2" value="开始查询"></p>
          <p>按图书的出版日期：<input type="text" name="book_time" placeholder="出版日期">&nbsp;<input type="submit" name="submit3" value="开始查询"></p>
          </td></tr>
        <tr>
          <td align="center" valign="top"><hr width="80%" color="#FFFFFF"/>           
<?php
include_once("conn/conn.php");
if(isset($_POST["submit1"])&& $_POST["submit1"]=="开始查询"){	
?> 
 <table width="780" > <tr bgcolor="#FFCC66"><td width="80" height="25" align="center"><span class="word5">编号</span></td>
    <td width="100" height="25" align="center"><span class="word5">书名</span></td>
    <td width="100" height="25" align="center"><span class="word5">价格</span></td>
    <td width="100" height="25" align="center"><span class="word5">出版日期</span></td>
    <td width="100" height="25" align="center"><span class="word5">图书类别</span></td>
    <td width="150" height="25" align="center"><span class="word5">ISBN编码</span></td>
    <td width="150" height="25" align="center"><span class="word5">出版社</span></td></tr>
<?php
	$sql = @mysqli_query($conn,"select * from tb_book");
    $info = mysqli_fetch_object($sql);
	...
	$pagesize = 3 ;									//每页显示记录数
    $sql = mysqli_query($conn,"select * from tb_book where book_type like '%".$book_type."%'"); //如果选择的条件为"like",则进行模糊查询
    $totalNum = mysqli_num_rows($sql);					//总记录数
	$pagecount = ceil($totalNum/$pagesize);						//总页数
	(!isset($_GET['page']))?($page = 1):$page = $_GET['page'];				//当前显示页数
	($page <= $pagecount)?$page:($page = $pagecount);//当前页大于总页数时把当前页定义为总页数
	$f_pageNum = $pagesize * ($page - 1);								//当前页的第一条记录
	$sqlstr1 = "select * from tb_book where book_type like '%".$book_type."%'"." limit ".$f_pageNum.",".$pagesize;//定义SQL语句，通过limit关键字控制查询范围和数量
	$result = mysqli_query($conn,$sqlstr1);								//执行查询语句
    ...
    do{
?>
        <tr align="left" bgcolor="#9999CC">
         <td width="80" height="20" align="center"><span class="word6"><?php echo $info->book_id; ?></span></td>
         <td width="100" align="center"><span class="word6"><?php echo $info->book_name; ?></span></td>
         <td width="100" align="center"><span class="word6"><?php echo $info->book_price; ?></span></td>
         <td width="100" align="center"><span class="word6"><?php echo $info->book_time; ?></span></td>
         <td width="100" align="center"><span class="word6"><?php echo $info->book_type; ?></span></td>
         <td width="150" align="center"><span class="word6"><?php echo $info->book_ISBN; ?></span></td>
         <td width="150" align="center"><span class="word6"><?php echo $info->book_come; ?></span></td>
        </tr>
<?php
     }while($info = mysqli_fetch_object($result));
?>
...
<?php
}else if(isset($_POST["submit2"])&& $_POST["submit2"]=="开始查询"){	
?> 
 <table width="780" > <tr bgcolor="#FFCC66"><td width="80" height="25" align="center"><span class="word5">编号</span></td>
    <td width="100" height="25" align="center"><span class="word5">书名</span></td>
    <td width="100" height="25" align="center"><span class="word5">价格</span></td>
...
<?php
	$sql = @mysqli_query($conn,"select * from tb_book");
    $info = mysqli_fetch_object($sql);
    $book_come = $_POST['book_come'];
	
	$pagesize = 3 ;									//每页显示记录数
    $sql = mysqli_query($conn,"select * from tb_book where book_come like '%".$book_come."%'"); //如果选择的条件为"like",则进行模糊查询
    $totalNum = mysqli_num_rows($sql);					//总记录数
	$pagecount = ceil($totalNum/$pagesize);						//总页数
	(!isset($_GET['page']))?($page = 1):$page = $_GET['page'];				//当前显示页数
	($page <= $pagecount)?$page:($page = $pagecount);//当前页大于总页数时把当前页定义为总页数
	$f_pageNum = $pagesize * ($page - 1);								//当前页的第一条记录
	$sqlstr1 = "select * from tb_book where book_come like '%".$book_come."%'"." limit ".$f_pageNum.",".$pagesize;//定义SQL语句，通过limit关键字控制查询范围和数量
	$result = mysqli_query($conn,$sqlstr1);								//执行查询语句
...
<?php
}else if(isset($_POST["submit3"])&& $_POST["submit3"]=="开始查询"){	
?> 
 <table width="780" > <tr bgcolor="#FFCC66"><td width="80" height="25" align="center"><span class="word5">编号</span></td>
    <td width="100" height="25" align="center"><span class="word5">书名</span></td>
    <td width="100" height="25" align="center"><span class="word5">价格</span></td>
...
<?php
	...
	$pagesize = 3 ;									//每页显示记录数
    $sql = mysqli_query($conn,"select * from tb_book where book_time like '%".$book_time."%'"); //如果选择的条件为"like",则进行模糊查询
    $totalNum = mysqli_num_rows($sql);					//总记录数
	$pagecount = ceil($totalNum/$pagesize);						//总页数
	(!isset($_GET['page']))?($page = 1):$page = $_GET['page'];				//当前显示页数
	($page <= $pagecount)?$page:($page = $pagecount);//当前页大于总页数时把当前页定义为总页数
	$f_pageNum = $pagesize * ($page - 1);								//当前页的第一条记录
	$sqlstr1 = "select * from tb_book where book_time like '%".$book_time."%'"." limit ".$f_pageNum.",".$pagesize;//定义SQL语句，通过limit关键字控制查询范围和数量
	$result = mysqli_query($conn,$sqlstr1);								//执行查询语句
