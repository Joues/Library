...
if ($_GET['action'] == "del"){							//判断是否执行删除
	$sqlstr1 = "delete from tb_bs where bs_id = ".$_GET['id'];		//定义删除语句
	$result = @mysqli_query($conn,$sqlstr1);				//执行删除操作
	if ($result){
		echo "<script>alert('删除成功');location='admin_borrowsend.php';</script>";
	}else{
...
if($_POST['action'] == "delall"){						//判断是否执行删除操作
	if(count($_POST['chk']) <= 0){						//判断提交的删除记录是否为空
		...
		for($i = 0; $i < count($_POST['chk']); $i++){		//for语句循环读取复选框提交的值，
			$sqlstr = "delete from tb_bs where bs_id = ".$_POST['chk'][$i];	//循环执行删除操作
